...
-- (เพิ่มการเก็บข้อมูลไฟล์และสติกเกอร์)
-- =====================================================
...
BEGIN
  ...
  INSERT INTO chat_messages (
    ...
    jsonb_build_object(
      ...
      -- ⭐ เพิ่ม file metadata
      'file_name', NEW.metadata->>'file_name',
      'fileName', NEW.metadata->>'fileName',
      'file_size', NEW.metadata->>'file_size',
      'fileSize', NEW.metadata->>'fileSize'
    )
  );
...
-- =====================================================
-- 
-- ✅ เพิ่มการเก็บข้อมูลสติกเกอร์:
-- - sticker_id
-- - sticker_package_id
-- - sticker_resource_type
-- - keywords (ใน metadata)
--
-- ✅ เพิ่มการเก็บข้อมูลไฟล์:
-- - file_name / fileName (ใน metadata)
-- - file_size / fileSize (ใน metadata)
--
-- ✅ ป้องกัน displayName และ pictureUrl
-- ✅ ป้องกันข้อความซ้ำ
-- ✅ อัพเดท chat_rooms อย่างปลอดภัย
--
-- =====================================================
